...
def render_route_html(route_info: dict) -> str:
    ...
    route_info: {"route": "semantic|hybrid|structured", "kg_count": n, "vec_count": m}
    """
    ...
    if route == "semantic":
        title = "语义检索通道"
        desc = "你的问题偏主观/体验类,知识图谱的结构化查询难以覆盖,已启用语义向量检索"
        active = "vector"
    elif route == "hybrid":
        title = "双引擎融合"
        desc = "你的问题既有结构化条件也有主观偏好,已同时启用图谱精确查询与语义检索,并用 RRF 融合排序"
        active = "both"
    else:
        return ""
...
<div class="rt-wrap">
  ...
  {f'<div class="rt-merge">↓ RRF 倒数排名融合 → LLM 精排取 Top-5</div>' if active == "both" else ''}
</div>
""").strip()
